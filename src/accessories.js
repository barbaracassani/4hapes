define(['jquery'], function ($) {
    ...
    SP.extend = function(parent, child, copyStatic) {

        var supa = parent.prototype;

        child.prototype = new parent();

        if ((copyStatic === undefined) ? true : copyStatic) {
            for (var i in parent) {
    ...
    SP.debounce = function(id, func, scope, time, params) {
        if (SP.debounces[id]) {
            return;
        }
        func.call(scope, params);
        SP.debounces[id] = window.setTimeout(function() {
            window.clearTimeout(SP.debounces[id]);
            delete SP.debounces[id];
        }, time);
    };
